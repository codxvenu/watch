...
app.post("/api/addcart", (req, res) => {
  ...
  if(results.length > 0){
    ...
    db.query('INSERT INTO cart(name,img,oprice,dprice,description,type,username,quantity) values(?,?,?,?,?,?,?,?)', [item.name,item.img,item.oprice,item.dprice,item.description,item.type,username,1], (err, result) => {
      if (err) {
        console.error('Database Error:', err);
